...
if ($T_Tipo == "insert") {
    ...
    <div class="modal-header">
        ...
        <h4 class="modal-title" id="modalTitle"><?php if ($o_Equipo->getId() == null) echo _("Agregar Equipo");
            else echo _("Agregar personas al Equipo"); ?></h4>
    </div>
                    ...
                    <select multiple id="selEquipos">
                        ...
                        if ($a_o_Personas != null) {
                            foreach ($a_o_Personas as $o_Persona) {
                                $checked = '';
                                // PERSONA ON DEVICE
                                if (in_array($o_Persona->getId(),$a_Personas_Equipo)) {
    ...
    <script type="text/javascript">
        ...
        function pagefunction() {
            ...
            $('#selEquipos').multiSelect({
                ...
                selectionHeader: "<legend><?php echo $o_Equipo->getDetalle(); ?></legend></br><section><i class='icon-prepend icon-prepend-multi-select-search fa fa-search'></i><input type='text' class='search-input' autocomplete='off' placeholder='Buscar'></section>",
        ...
        function AgregarPersona(persona) {
            ...
            $.ajax({
                url: 'ajax/equipos-personas.html.php?tipo=insert&id=<?php echo $T_Id; ?>&personaID=' + persona,
                dataType: 'html',
                async: true,
                ...
                success: function (text) {
                    ...
                    if (text) {
                        if (text == "error_persona_ya_existe") {
                            alert("<?php echo _('La persona que intenta agregar, ya existe en el equipo.') ?>");
                            $('#loader').html('');
        ...
        function EliminarPersona(persona) {
            ...
            $.ajax({
                url: '<?php echo 'ajax/equipos-personas.html.php' ?>?tipo=remove&id=<?php echo $T_Id; ?>&personaID=' + persona,
                dataType: 'html',
                async: true,
                timeout: 5000, //timeout de 5 segundos
                success: function (text) {
