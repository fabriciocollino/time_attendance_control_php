...
if ($T_Intervalo = 'F_Hoy'){
    $T_Intervalo    =   array(
        'desde' => date('Y-m-d H:i:s', strtotime('yesterday 17:00:00'))  ,
...
$a_Salidas_Temprano     = $o_Reporte->generar_reporte();

unset($o_Reporte);



foreach ($a_Salidas_Temprano as $perID => $item){
    $cantidad += $item['Total'];
}
...
if ($cantidad > 0) {?>
    ...
    <div>
        <table id="dt_basic"
            ...
            <?php foreach ($a_Salidas_Temprano as $per_Id => $item){?>
                ...
                <tr>
                    ...
                    <!-- HORA -->
                    <td>
                        <?php foreach ($item['Salidas_Temprano'] as $key => $log){ ?>

                            <div>
                                        <span class="">
                                            <?php
                                            $hora       = date('H:i', strtotime($log['Hora_Fin']) );
                                            $dia        = $dias[date('w', strtotime($log['Fecha_Fin']))];
...
<script type="text/javascript">
    ...
    $("#div_unread_count_salidas_temprano").text("<?php echo $cantidad; ?>");
