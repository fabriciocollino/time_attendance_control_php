...
class Suspensions_O
{
    ...
    public function loadArray($p_Datos) {
        ...
        $this->_id = (integer)$p_Datos["sus_Id"];
        $this->_per_id = (integer)$p_Datos["sus_Per_Id"];
        $this->_gru_id = (integer)$p_Datos["sus_Gru_Id"];
        $this->_tipo = (integer)$p_Datos["sus_Tipo"];
        $this->_fecha_inicio = (string)$p_Datos["sus_Fecha_Inicio"];
        $this->_fecha_fin = (string)$p_Datos["sus_Fecha_Fin"];
        $this->_duracion = (string)$p_Datos["sus_Duracion"];
        $this->_motivo = (string)$p_Datos["sus_Motivo"];
        $this->_repetitiva = (integer)$p_Datos["sus_Repetitiva"];
        $this->_enabled = (integer)$p_Datos["sus_Enabled"];
        $this->_fecha_mod = (string)$p_Datos["sus_Fecha_Mod"];
    ...
    public function saveEnabled($status){
        ...
        $resultado = $cnn->Update('suspensions', $datos, "sus_Id = $id");
    }
    //Rahul
    public function save($p_Debug) {
        ...
        $datos = array(
            'sus_Per_Id' => $this->_per_id,
            'sus_Gru_Id' => $this->_gru_id,
            'sus_Tipo' => $this->_tipo,
            'sus_Fecha_Inicio' => $this->_fecha_inicio,
            'sus_Fecha_Fin' => $this->_fecha_fin,
            'sus_Duracion' => $this->_duracion,
            'sus_Motivo' => $this->_motivo,
            'sus_Repetitiva' => $this->_repetitiva,
            'sus_Enabled' => $this->_enabled,
            'sus_Fecha_Mod' => $this->_fecha_mod

        );

        if ($this->_id == 0) {
            $resultado = $cnn->Insert('suspensions', $datos);
            if ($resultado !== false) {
                $this->_id = $cnn->Devolver_Insert_Id();
            }
        } else {
            $resultado = $cnn->Update('suspensions', $datos, "sus_Id = {$this->_id}");
        }
    ...
    public function delete($p_Debug) {
        ...
        if ($cnn->Delete('suspensions', "sus_Id = {$this->_id}")) {
            return true;
        } else {
            $this->_errores['mysql'] = $cnn->get_Error($p_Debug);
            return false;
