...
switch ($T_Tipo) {
    ...
    case 'edit':
        ...
        if (!$o_Horario_Flexible->save(Registry::getInstance()->general['debug'])) {
            $T_Error = "Lo sentimos, hubo un error en la operación.";//$o_Horario_Flexible->getErrores();
        } else {
            if($nuevo_horario)
            ...
            $T_Mensaje = _('Horario flexible guardado correctamente.');
        }
        ...
        if (is_null($o_Horario_Flexible)) {
            ...
            if ($cantidad_personas == 0) {
                if (!$o_Horario_Flexible->delete(Registry::getInstance()->general['debug'])) {
                    ...
                    $T_Mensaje = _('El horario flexible fue eliminado correctamente.');
                }
            } else {
                $T_Eliminado = false;
...
$_SESSION['filtro']['tipo_data']             =   'Horarios_Flexibles';
