...
switch ($T_Tipo) {
    ...
    case 'edit':
        ...
        if (!$o_Horario_Multiple->save(Registry::getInstance()->general['debug'])) {
            ...
            $T_Mensaje = _('Horario múltiple guardado correctamente.');
        }
    ...
    case 'delete':
        ...
        if (is_null($o_Horario_Multiple)) {
            $T_Error = _('Lo sentimos, el horario múltiple que desea eliminar no existe.');
        } else {
            ...
            if ($cantidad_personas == 0) {
                if (!$o_Horario_Multiple->delete(Registry::getInstance()->general['debug'])) {
                    ...
                    $T_Mensaje = _('El horario múltiple fue eliminado correctamente.');
                }
            } else {
                $T_Eliminado = false;
