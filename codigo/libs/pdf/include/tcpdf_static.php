	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function extractCSSproperties($cssdata) {
		...
		foreach ($cssblocks as $block) {
			...
			$c += intval(preg_match_all('/[\:]link|visited|hover|active|focus|target|locale|enabled|disabled|checked|indeterminate|root|nth|first|last|only|empty|contains|not/i', $selector, $matches)); // number of pseudo-classes
					...
					switch ($attrib{0}) {
						...
						case ':': { // pseudo-class or pseudo-element
							if ($attrib{1} == ':') { // pseudo-element
								...
								// (:root, :nth-child(n), :nth-last-child(n), :nth-of-type(n), :nth-last-of-type(n), :first-child, :last-child, :first-of-type, :last-of-type, :only-child, :only-of-type, :empty, :link, :visited, :active, :hover, :focus, :target, :locale(fr), :enabled, :disabled, :checked)
							}
							break;
						}
					} // end of switch
